...
const CetakLaporanSemester: React.FC<Props> = ({ siswaId }) => {
  ...
  const getLaporan = async () => {
    ...
    const { data } = await api.get(`/rekap/semester-laporan?siswaId=${selectedSiswa}`);
    setSiswaDetail(data.siswa);
    console.log("REKAP RESPONSE:", data.rekap);
    setRekap(
      semester === "Semua"
        ? data.rekap
        : data.rekap.filter((item: RekapItem) => item.semester.toString() === semester)
    );
  };
      ...
      {rekap.length > 0 && siswaDetail && (
        <>
          <div ref={componentRef} className="w-full flex flex-col gap-5 py-10 px-10">
            <h2 className="text-center text-2xl font-bold">Laporan Penilaian Semester</h2>
            <table className="w-full min-w-full bg-white border border-gray-400 rounded-xl shadow">
              <tbody className="border border-gray-400">
                <tr className="h-16 border-b border-gray-400">
                  ...
                  <td className="align-top px-4 py-2">{semester}</td>
                </tr>
                <tr className="h-16 border-b border-gray-400">
            ...
            <TabelDetailSemester data={rekap} filterSemester={semester} showNo={true} showSemester={false} />
...
export default CetakLaporanSemester;
