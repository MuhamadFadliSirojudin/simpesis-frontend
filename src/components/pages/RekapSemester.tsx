...
const TabRekapSemester = ({ onLihatRekap, onCetakLaporan }: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await api.get("/rekap/semester");
        ...
        console.error("Gagal mengambil data rekap semester:", err);
      }
    };
    fetchData();
  }, []);
  ...
  return (
    <div className="w-full gap-10 shadow rounded-lg bg-[#f4f4f9] p-8">
      <h2 className="text-xl text-center font-semibold mb-4">Rekap Penilaian Semester</h2>
            ...
            {dataSiswa.length === 0 && (
              <tr>
                <td colSpan={5} className="text-center py-4">
                  Tidak ada data rekap semester ditemukan.
                </td>
              </tr>
            )}
          </tbody>
...
export default TabRekapSemester;
